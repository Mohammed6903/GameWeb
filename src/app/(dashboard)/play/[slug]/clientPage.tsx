...
export default function GamePage({ game, user, adSetting }: GamePageProps) {
  ...
  return (
    <>
      ...
      />
      <main className="min-h-screen bg-gradient-to-br from-[#120a2c] to-[#1e1540] text-white p-6 md:p-8 lg:p-12">
        <div className="container mx-auto px-4 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            {/* Main Game Content */}
            <div className="lg:col-span-3 space-y-6">
              ...
              <header className="flex justify-between items-center">
                <Link href="/">
                  <ArrowLeft size={24} aria-label="Back to home" />
                </Link>
                <h1 className="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-blue-600">
                  {game.name}
                </h1>
              ...
              {/* Game Viewer */}
              <section className="rounded-2xl overflow-hidden shadow-2xl border-4 border-purple-800/50">
                <GameViewer play_url={game.play_url} thumbnail={game.thumbnail_url} game_id={game.id} />
              </section>
              ...
              {/* Comments Section */}
              <section id="commentSection" aria-label="Game Comments">
                <CommentSection gameId={game.id} />
              </section>
            </div>
